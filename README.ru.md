...
По умолчанию, avbroot использует потоковую запись для вывода OTA во время патчинга. Это означает, что он вычисляет дайджест sha256 для цифровой подписи одновременно с записью файла. Такой режим приводит к тому, что в ZIP-файле появляются дескрипторы данных, что является частью стандарта ZIP и работает на подавляющем большинстве устройств. Однако некоторые устройства имеют некорректно работающие парсеры ZIP-файлов, которые не могут правильно прочитать ZIP-файлы OTA, содержащие дескрипторы данных. Если это ваш случай, используйте параметр `--zip-mode seekable` при патчинге.

Режим поиска (seekable mode) записывает ZIP-файлы без дескрипторов данных и выполняет перемещение по файлу, вместо последовательной записи. Дайджест sha256 для цифровой подписи вычисляется уже после того, как ZIP-файл был полностью записан.
...
Дебаг-сборки тоже работают, но значительно медленнее (в вычислениях sha256), потому что отключены оптимизации компилятора.
